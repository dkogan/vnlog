...
with =make=. Note: GNU Make is required.
...
** Installation on non-Debian-based boxes
...
Note: this requires the =chrpath= tool, which is available in most package
repositories.
...
COMPATIBILITY
    This calls the tool named "ts". At least on FreeBSD, it's called
    "moreutils-ts", however. Currently only the nominal name is used, so you
    need to make the tool be available under the name "ts" to make this
    work.
