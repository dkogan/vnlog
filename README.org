...
DESCRIPTION
    ...
    *   "vnl-join"-specific options are available to adjust the field-naming
        in the output: "--vnl-prefix1", "--vnl-prefix2", "--vnl-suffix1",
        "--vnl-suffix2". See below for details.
...
At this time vnlog is a part of Debian/sid, and will migrate to the latest
Ubuntu release in the near future. On those boxes you can simply

#+BEGIN_EXAMPLE
$ sudo apt install vnlog libvnlog-dev libvnlog-perl libvnlog-python
#+END_EXAMPLE

to get the binary tools, the C API, the perl and python interfaces respectively.
...
$ sudo dpkg -i ../vnlog*.deb ../libvnlog-dev*.deb ../libvnlog-perl*.deb ../libvnlog-python*.deb
#+END_EXAMPLE
...
#+BEGIN_EXAMPLE
$ make
$ DESTDIR=/home/goober/local make install
#+END_EXAMPLE
