...
will not have data about any other vehicles, and vice versa. =vnl-filter= can be
used to pull out the different subsets. Sample =joint.vnl=:
...
Here we have 3 instances in time. We have no observations of other vehicles at
=time= 1, one observation at =time= 2, and two observations at =time= 3. We can
use =vnl-filter= to pull out the data we want. The =+= modifier in picked
columns in =vnl-filter= is especially useful here:
...
$ < joint.vnl vnl-filter -p time,+other
...
$ vnl-join -j time -a- self.vnl other.vnl
...
The tools that wrap standard commandline utilities (=vnl-sort=, =vnl-uniq=,
=vnl-join=, =vnl-tail=, =vnl-ts=) are written specifically to work with the
Linux kernel and the GNU coreutils. None of these have been tested with BSD
tools or with non-Linux kernels, and I'm sure things don't just work. It's
...
DESCRIPTION
    ...
    *   One *one* input is supported (a file on the cmdline or data on
